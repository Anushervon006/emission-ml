"""
ALL-k PERFORMANCE + RAW FOLD-LEVEL FEATURE-SELECTION STABILITY
==============================================================

Required methodology
--------------------
1. Run 10-fold CV for every feature-selection Method × k.
2. Perform feature selection inside each training fold.
3. Preserve the 10 original fold-level subsets exactly as selected.
4. Calculate all stability metrics directly from those 10 raw subsets for
   every Method × k. No 10/10 filtering, consensus replacement, or subset
   modification is applied before stability calculation.
5. Choose optimal k separately for RF and GB from predictive performance.
6. Mark each selected optimal k on the RF and GB performance figures.
7. Produce all-k stability heatmaps for every feature-selection method.
8. Produce compact RF/GB optimal-k summary tables/figures.
9. Report and verify Pearson and ANOVA-F descriptors at k=7.

Important
---------
Feature selection depends on Method, k, and CV training fold, but not on the
later RF/GB regressor. Therefore, stability for the same Method × k is shared
by RF and GB. RF and GB may still select different optimal k values.
"""
...
OUTPUT_DIR = r"C:\\Users\\anush\Desktop\\ML projects\\separeted by number of dopands\\Final_Results_ALL_K"
...
# ============================================================================
# 7. PEARSON / ANOVA-F k=7 EXACT-DESCRIPTOR VERIFICATION
# ============================================================================
...
if __name__ == "__main__":
    ...
    optimal_df = select_optimal_k(performance_df, stability_df)
    k7_confirmation = verify_pearson_anova_k7(fold_subsets_df)

    plot_performance_sweep(performance_df, optimal_df)
    plot_stability_heatmaps(stability_df)
    plot_summary_table(optimal_df)
    ...
    print(
        "All stability values were calculated directly from the 10 original "
        "fold-level subsets for each Method × k. No 10/10 filtering or "
        "consensus replacement was applied."
    )
